...
const App = (() => {
  ...
  function renderWorkIndex() {
    ...
    const rows = DATA.categorias.map((cat, i) => {
      ...
      const first = projs[0];
      // Rotación: rutas directas (previewSrcs), curada por números (previews), o portada
      let previews = [];
      if (cat.previewSrcs) {
        previews = cat.previewSrcs;
      } else if (cat.previews && first) {
      ...
      const href = cat.href || `categoria.html?cat=${cat.slug}`;
      return `
        <a class="work-row" href="${href}" data-previews="${previews.join("|")}">
    ...
    const sizeTo = (img) => {
      ...
      preview.style.width = Math.round(img.naturalWidth * s) + "px";
      preview.style.height = Math.round(img.naturalHeight * s) + "px";
    };
    imgA.addEventListener("load", () => { if (front === imgA) sizeTo(imgA); });
    imgB.addEventListener("load", () => { if (front === imgB) sizeTo(imgB); });
    const crossfade = () => {
      ...
      if (front.complete && front.naturalWidth) sizeTo(front);
    };
    ...
    wrap.querySelectorAll(".work-row").forEach((row) => {
      row.addEventListener("mouseenter", () => {
        ...
        stopRot();
        if (srcs.length > 1) rot = setInterval(crossfade, 1400);
      });
  ...
  function renderFeatured() {
    ...
    wrap.innerHTML = feats.map((p) => {
      ...
      const href = p.href || `proyecto.html?id=${p.id}`;
      return `
        <a class="feature reveal" href="${href}">
          ...
          <div class="feature__body">
            <div>
              <p class="feature__cat">${p.kicker || (cat ? cat.nombre : "")} — ${p.anio || ""}</p>
  ...
  function renderCategoria() {
    ...
    const cat = DATA.categorias.find((c) => c.slug === slug) || DATA.categorias.find((c) => !c.href);
    if (!cat) { wrap.innerHTML = "<p class='prose'>Categoría no encontrada.</p>"; return; }
    if (cat.href) { location.replace(cat.href); return; }
